...
export function ChatInterface({
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const el = document.querySelector<HTMLElement>("[data-radix-scroll-area-viewport]")
      if (el) {
        const handleScroll = () => {
          ...
          if (atBottom) {
            ...
            const fraction = Math.max(0, (distanceFromBottom - 10) / (FADE_DISTANCE - 10))
        ...
        <div className="relative h-full">
          <ScrollArea className="absolute h-full w-full p-4">
            <div className="space-y-4">
              {messages.map((message, index) => (
                <div key={index} className={`flex items-start gap-3 ${message.type === "user" ? "justify-end" : "justify-start"}`}>
                  ...
                  <div className={`max-w-[80%] rounded-lg p-3 text-sm ${message.type === "user" ? "bg-blue-500 text-white" : "bg-gray-100 text-gray-900 whitespace-pre-wrap"}`}>
            ...
            <div className="absolute bottom-6 right-6 z-20">
              <Button variant="secondary" className="h-[40px] w-[40px] rounded-full bg-gradient-to-br from-indigo-500 to-purple-500 p-0 text-white shadow-lg transition-opacity hover:from-indigo-600 hover:to-purple-600 disabled:cursor-not-allowed" style={{ boxShadow: "0 4px 24px 0 rgba(80, 0, 200, 0.15)", opacity: scrollButtonOpacity }} onClick={() => chatEndRef.current?.scrollIntoView({ behavior: "smooth", block: "start" })} aria-label="Rolar para o fim do chat" disabled={!canScrollToEnd}>
                <ArrowDownCircle className={`h-8 w-8 ${canScrollToEnd ? "animate-bounce" : ""}`} />
              </Button>
            </div>
          </ScrollArea>
        </div>
