...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!incomingData.birthDate || !incomingData.birthTime ||
        typeof incomingData.latitude !== 'number' || typeof incomingData.longitude !== 'number' ||
        typeof incomingData.timezone !== 'number') {
      return NextResponse.json({ error: "Dados de entrada inválidos ou ausentes." }, { status: 400 });
    }
    ...
    console.error("Erro no catch de /api/calculate:", error);
    if (error instanceof Error) {
      return NextResponse.json({ error: error.message }, { status: 500 });
    }
    return NextResponse.json({ error: "Erro interno ao calcular mapa astral" }, { status: 500 });
  }
}
